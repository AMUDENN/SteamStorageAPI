...
- Composite indexes on `(SkinId, DateUpdate)`, `(CurrencyId, DateUpdate)`, `(GroupId, DateUpdate)` — used by dynamics
  queries
- A SQL trigger `UpdateSkinsCurrentPrice` on `SkinsDynamic` keeps `Skin.CurrentPrice` in sync without a separate UPDATE
  query
...
Shared numeric limits (e.g. `MaxPrice = 1 000 000 000 000`) live in `ValidationConstants` and are imported by all
relevant validators.
...
`/api/metrics` is protected by a static Bearer token (`internalApiKey`). Standard JWT validation is not used here
because Prometheus does not support JWT. Configure the same key in `prometheus.yml` under `authorization.credentials`.

### Admin endpoints

Admin-only endpoints (`PostCurrency`, `SetRole`, `TriggerJob`, etc.) require the caller's JWT to carry the `Admin` role
claim, enforced via policy-based authorization.
